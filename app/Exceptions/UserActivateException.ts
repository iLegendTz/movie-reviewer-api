...
|
| @example
| new UserActivateException('message', 500, 'E_RUNTIME_EXCEPTION')
|
*/
export default class UserActivateException extends Exception {
  /**
  ...
  public async handle(error: this, ctx: HttpContextContract) {
    return ctx.response.status(error.status || 500).send({ message: 'El link ha expirado o es invalido' })
  }
}
